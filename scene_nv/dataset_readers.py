...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder): #(L.T.W.O.F)
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=None,                             # earlier it was image=image (P.V.C.A.P)
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    try:
        ...
        cameras_extrinsic_file = os.path.join(path, "images.txt") #(L.T.W.O.F)
        cameras_intrinsic_file = os.path.join(path, "cameras.txt")
        cam_extrinsics = read_extrinsics_text(cameras_extrinsic_file) #(L.T.W.O.F)
        cam_intrinsics = read_intrinsics_text(cameras_intrinsic_file)

    reading_dir = "images" if images == None else images
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir)) #(L.T.W.O.F)
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.uid)  ## x : x.image_name (changed by Pranav)
